...
function mim_sms_prefijo( $servicio ) {
	$prefijo = array( 
		"mimsms_esms",
		"mimsms_dotbd",		
	);
...
function mim_sms_normaliza_mensaje( $mensaje ) {
	$reemplazo = array( 
		'Š'			=> 'S', 
		'š'			=> 's', 
		'Đ'			=> 'Dj', 
...
//Codifica el mensaje
function mim_sms_codifica_el_mensaje( $mensaje ) {
	return apply_filters( 'mim_sms_message_return', urlencode( html_entity_decode( $mensaje, ENT_QUOTES, "UTF-8" ) ), $mensaje);
}

//Procesa el teléfono y le añade, si lo necesita, el prefijo
function mim_sms_procesa_el_telefono( $pedido, $telefono, $servicio, $propietario = false, $envio = false ) {
	$telefono_procesado = $telefono;
	
	if ( empty( $telefono_procesado ) ) { //Control
		return apply_filters( 'mim_sms_phone_return', $telefono_procesado, $pedido, $telefono, $servicio, $propietario, $envio );
	}
	
	//Permite que otros plugins impidan que se procese el número de teléfono
	if ( apply_filters( 'mim_sms_phone_process', true, $pedido, $telefono, $servicio, $propietario, $envio ) ) {
		...
		$prefijo				= mim_sms_prefijo( $servicio );
		...
		if ( !$propietario ) {
			if ( ( !$envio && $billing_country && ( WC()->countries->get_base_country() != $billing_country ) || $prefijo ) ) {
				$prefijo_internacional = mim_sms_dame_prefijo_pais( $billing_country ); //Teléfono de facturación
			} else if ( ( $envio && $shipping_country && ( WC()->countries->get_base_country() != $shipping_country ) || $prefijo ) ) {
				$prefijo_internacional = mim_sms_dame_prefijo_pais( $shipping_country ); //Teléfono de envío
			}
		} else if ( $propietario && $prefijo ) {
			$prefijo_internacional = mim_sms_dame_prefijo_pais( WC()->countries->get_base_country() );
		}
...
function mim_sms_procesa_variables( $mensaje, $pedido, $variables, $nota = '' ) {
	...
	$mim_sms_variables = array( //Hay que añadirles un guión
		"order_key", 
		"billing_first_name", 
		"billing_last_name", 
		"billing_company", 
	...
	foreach ( $busqueda[1] as $variable ) { 
		...
		if ( !in_array( $variable, $mim_sms ) && !in_array( $variable, $mim_sms_variables ) && !in_array( $variable, $variables_personalizadas ) ) {
			continue;
		}
		...
		if ( !in_array( $variable, $especiales ) ) {
			...
			} else if ( in_array( $variable, $mim_sms_variables ) ) {
			...
			if ( !isset( $mimsms_settings['productos'] ) || $mimsms_settings['productos'] != 1 ) {
	...
	return apply_filters( 'mim_sms_message' , $mensaje , $numero_de_pedido );
}

//Devuelve el código de prefijo del país
function mim_sms_dame_prefijo_pais( $pais = '' ) {
	$paises = array( 
		'AC' => '247', 
		'AD' => '376', 
		'AE' => '971', 
